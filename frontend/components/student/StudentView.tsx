import {Answer, Position, StatusSuggestion, Student, StudentBase, Url, User} from "../../lib/types";
...
import {getUrlMap, getUrlList} from "../../lib/requestUtils";
...
async function getEntireStudent(
    ...
    const coachUrls = new Set<Url>();
    const coachMap = new Map<Url, User>();
    newStudent.statusSuggestions.forEach(suggestion => coachUrls.add('http://localhost:8080/api/users/' + suggestion.coachId));
    console.log(JSON.stringify(coachUrls));

    const temp = Array.from(coachUrls.values())
    ...
    console.log("coachMap:", JSON.stringify(coachMap));

    // once backend fixes this
    // for (const suggestion of newStudent.statusSuggestions) {
    //     suggestion.coach = coachMap.get(
    //         suggestion.coach as unknown as string
    //     ) as User;
    // }

    return [newStudent, coachMap];
}

const StudentView: React.FC<StudentViewProp> = ({studentInput}: StudentViewProp) => {
    ...
    const [coachMap, setCoachMap] = useState(new Map<Url, User>());
    ...
    useEffect(() => {
        ...
        if (studentInput.id !== undefined){
            getEntireStudent(studentInput, signal, setError, router).then((response) => {
                    ...
                    >
                        <Fragment>
                            ...
                            <Select
                                ...
                                options={[{value: '', label: 'Yes'}, {value: '', label: 'No'}, {value: '', label: 'Maybe'}, {value: '', label: 'Undecided'}]}
                                // value={skills}
...
const StudentStatusSuggestion: React.FC<StatusSuggestionProp> = ({statusSuggestion, coachMap}: StatusSuggestionProp) => {
    console.log('statussugg:', JSON.stringify(statusSuggestion));
    ...
    return (
        <div className="flex flex-row">
            ...
            <p className="">{coachMap.get('http://localhost:8080/api/users/' + statusSuggestion.coachId)}</p>
        </div>
    );
};
